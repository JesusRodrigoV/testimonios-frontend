...
export default class TestimonyUploadComponent implements OnInit {
  ...
  testimony = signal<
    TestimonyInput & { selectedTags: string[]; selectedCategories: string[] }
  >({
    title: "",
    description: "",
    content: "",
  ...
  set shareLocation(value: boolean) {
    this._shareLocation.set(value);
    this.locationService.shareLocation = value;
  }
  ...
  constructor() {
    ...
    this.locationService.location$.pipe(takeUntil(this.destroy$)).subscribe({
      next: (location) => {
        if (
          ...
          this.showError("obtener ubicación", new Error(location.error));
        }
      },
    });
  }
  ...
  submitTestimony(form: NgForm): void {
    if (!this.isFileUploaded() || !form.valid) {
      this.showError(
        "enviar testimonio",
        new Error("Archivo o formulario inválido")
      );
      return;
    }
  ...
  addTag(event: MatChipInputEvent): void {
    ...
    if (value && !this.testimony().selectedTags.includes(value)) {
      this.testimony.update((t) => ({
        ...t,
        selectedTags: [...t.selectedTags, value],
      }));
    }
  ...
  private showError(action: string, error: Error): void {
    this.snackBar.open(`Error al ${action}: ${error.message}`, "Reintentar", {
      duration: 5000,
  ...
  private getErrorMessage(err: any): string {
    ...
    if (message.includes("url"))
      return "Error con el archivo multimedia. Reintenta subirlo.";
    if (message.includes("eventId"))
      return "Error con el evento seleccionado. Verifica la selección.";
    if (message.includes("Invalid type"))
      return "Error en datos numéricos. Déjalos en blanco si no aplican.";
    return message;
  }
}
