...
export class TestimonioService {
  ...
  getAll(): Observable<Testimony[]> {
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          return throwError(
            () => new Error(error.message || "Error al obtener testimonios"),
          );
        }),
      );
  }
  ...
  rateTestimony(testimonyId: number, rating: number): Observable<number> {
    return this.http.post<number>(`${this.apiUrl}/${testimonyId}/rate`, {
      rating,
    });
  }
    ...
    return this.http.get(testimony.url, { responseType: "blob" }).pipe(
      catchError((error) => {
        return throwError(
          () =>
            new Error(
              error.message || "Error al descargar el archivo desde Cloudinary",
            ),
        );
      }),
    );
  ...
  getTranscription(testimony: Testimony): Observable<string> {
    return this.http.get<string>(
      `${this.apiUrl}/${testimony.id}/transcription`,
    );
  }
  ...
  suggestImprovement(
    ...
    return this.http.post<void>(`${this.apiUrl}/${testimonyId}/suggestions`, {
      field,
      value,
    });
  }
